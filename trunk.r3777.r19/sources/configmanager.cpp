...
bool ConfigManager::load()
{
	...
	if(!m_loaded) //info that must be loaded one time- unless we reset the modules involved
	{
		...
		m_confBool[TRUNCATE_LOG] = getGlobalBool("truncateLogOnStartup", false);
	...
	m_confNumber[ROOK_LEVELTO] = getGlobalNumber("rookLevelTo", 6);
	m_confNumber[ROOK_TOWN] = getGlobalNumber("rookTownId", 1);
	m_confBool[ROOK_SYSTEM] = getGlobalBool("rookSystem", true);
	m_confNumber[DEFAULT_DEPOT_SIZE] = getGlobalNumber("depotItemsFreeLimit", 500);
	m_confNumber[DEFAULT_DEPOT_SIZE_PREMIUM] = getGlobalNumber("depotItemstPremiumLimit", 1000);
	m_confBool[USE_MC] = getGlobalBool("useMultiClient", false);
	m_confBool[ENABLE_CAP] = getGlobalBool("useCapSystem", true);
	m_confBool[USE_SHARE_MONSTER_EXP] = getGlobalBool("NoShareExpSummonMonster", false);
	m_confBool[BLOCK_RESPAWN] = getGlobalBool("NoDisableBlockRespawn", true);
	...
	m_confBool[MONSTER_SPAWN_WALKBACK] = getGlobalBool("monstersGoBackToSpawn", false);

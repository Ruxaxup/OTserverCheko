...
void Items::parseItemNode(xmlNodePtr itemNode, uint32_t id)
{
	...
	while(itemAttributesNode)
	{
		if(readXMLString(itemAttributesNode, "key", strValue))
		{
			...
			else if(tmpStrValue == "pickupable" || tmpStrValue == "blockprojectile")
			{
				if(readXMLInteger(itemAttributesNode, "value", intValue))
	...
	it.abilities.absorb[COMBAT_FIRST] = it.abilities.absorb[COMBAT_FIRST + 1];
	it.abilities.reflect[REFLECT_PERCENT][COMBAT_FIRST] = it.abilities.reflect[REFLECT_PERCENT][COMBAT_FIRST + 1];
	it.abilities.reflect[REFLECT_CHANCE][COMBAT_FIRST] = it.abilities.reflect[REFLECT_CHANCE][COMBAT_FIRST + 1];
	for(uint32_t i = (COMBAT_FIRST + 1) << 1; i <= COMBAT_LAST; i <<= 1)
	{
		if(it.abilities.absorb[COMBAT_FIRST] != it.abilities.absorb[i])
			it.abilities.absorb[COMBAT_FIRST] = 0;

		if(it.abilities.reflect[REFLECT_PERCENT][COMBAT_FIRST] != it.abilities.reflect[REFLECT_PERCENT][i])
			it.abilities.reflect[REFLECT_PERCENT][COMBAT_FIRST] = 0;

		if(it.abilities.reflect[REFLECT_CHANCE][COMBAT_FIRST] != it.abilities.reflect[REFLECT_CHANCE][i])
			it.abilities.reflect[REFLECT_CHANCE][COMBAT_FIRST] = 0;
	}
}

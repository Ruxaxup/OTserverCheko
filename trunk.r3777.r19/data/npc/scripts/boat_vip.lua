apocalypseremoveaki = {x=759, y=678, z=7}
...
function onCreatureSay(cid, type, msg)
	...
	if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 4 then
  		selfSay('Hiho ' .. getCreatureName(cid) .. '! 	Anda logo, me diga a senha')
  		focus = cid
  		talk_start = os.clock()
  	...
  	elseif focus == cid then
		talk_start = os.clock()
			...
			if msgcontains(msg, 'apocalypseremoveaki') then
				if(doPlayerRemoveMoney(cid, 50) == TRUE) then
					doTeleportThing(cid,apocalypseremoveaki)
    					doSendMagicEffect(apocalypseremoveaki,10)
					selfSay("Vai")
					focus = 0
					talk_start = 0
				else
